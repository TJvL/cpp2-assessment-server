...
namespace cpp2 {
    Server::Server(const int serverPort, const std::string &syncDirectoryPath, const bool canRemoteTerminate)
            : serverPort(serverPort), fileSystemManager(syncDirectoryPath), canRemoteTerminate(canRemoteTerminate) {}
    ...
    void Server::handleClientConnection() const {
        ...
        while (handlingConnection) {
            try {
                ...
                if (canRemoteTerminate && message == REMOTE_TERMINATION) {
